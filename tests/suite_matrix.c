...
START_TEST(det_test_one_dim) {
  matrix_t a;
  double b;
  s21_create_matrix(1, 1, &a);
  a.matrix[0][0] = 0.22;
  s21_determinant(&a, &b);
  ck_assert_double_eq_tol(b, 0.22, 1e-6);
  s21_remove_matrix(&a);
}
END_TEST
...
START_TEST(incorrect_create) {
  int res = s21_create_matrix(7, 4, NULL);
...
START_TEST(incorrect_mul_num) {
  matrix_t A, B;
  s21_create_matrix(7, 4, &A);
  s21_create_matrix(7, 4, &B);
  ck_assert_int_eq(s21_mult_number(NULL, 7.2, &B), 1);
  s21_remove_matrix(&A);
  s21_remove_matrix(&B);
}
END_TEST

START_TEST(incorrect_eq_row_columns) {
  matrix_t A, B;
...
START_TEST(incorrect_calc_null) {
  matrix_t B;
  ck_assert_int_eq(s21_calc_complements(NULL, &B), 1);
}
END_TEST

START_TEST(incorrect_transpose) {
  matrix_t A, B;
  s21_create_matrix(7, 3, &A);
  ck_assert_int_eq(s21_transpose(NULL, &B), 1);
  s21_remove_matrix(&A);
}
END_TEST
...
START_TEST(incorrect_det) {
  ...
  double d;
  s21_create_matrix(7, 4, &A);
  ck_assert_int_eq(s21_determinant(&A, &d), 2);
  s21_remove_matrix(&A);
}
END_TEST

START_TEST(incorrect_det_2) {
  matrix_t A;
  double d;
  s21_create_matrix(7, 4, &A);
  ck_assert_int_eq(s21_determinant(NULL, &d), 1);
  s21_remove_matrix(&A);
}
END_TEST

START_TEST(incorrect_eq) {
...
START_TEST(incorrect_inverse_2) {
  matrix_t A, B;
  s21_create_matrix(7, 4, &A);
  ck_assert_int_eq(s21_inverse_matrix(NULL, &B), 1);
  s21_remove_matrix(&A);
}
END_TEST
...
Suite *suite_matrix() {
  ...
  tcase_add_test(tc1_1, incorrect_create);
  tcase_add_test(tc1_1, incorrect_inverse_2);

  tcase_add_test(tc1_1, incorrect_eq);
  tcase_add_test(tc1_1, incorrect_mul_num);
  tcase_add_test(tc1_1, incorrect_eq_row_columns);
  tcase_add_test(tc1_1, incorrect_transpose);
  tcase_add_test(tc1_1, incorrect_det_2);
  tcase_add_test(tc1_1, det_test_one_dim);
  tcase_add_test(tc1_1, incorrect_calc_null);
